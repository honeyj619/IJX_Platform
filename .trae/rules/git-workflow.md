...
**提交代码（git commit）和拉取代码（git pull）必须由用户明确指示后我才操作，不要自动执行！**
...
1. **每日开始**：先 pull 最新代码（用户指示）
2. **频繁提交**：每完成一个小功能就提交（用户指示）
3. **清晰信息**：使用语义化的提交信息
4. **及时推送**：避免本地积累太多提交（用户指示）
5. **定期同步**：每隔一段时间拉取最新代码（用户指示）

### 6. 注意事项

- ⚠️ **使用 `--force` 推送时要格外小心，必须用户明确要求**
- ⚠️ **Rebase 会重写提交历史，不要对已共享的提交使用**
- ⚠️ **解决冲突时要仔细，确保代码功能正常**
- ⚠️ **提交和拉取操作必须用户明确指示才执行**
