        ...
        <div className="bg-white border-b border-slate-200/60 py-3 px-6 w-full">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 w-full">
            <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl">
              <button 
                className={`px-5 py-2 text-sm font-medium rounded-lg transition-all ${activeTab === 'schedule' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
                onClick={() => setActiveTab('schedule')}
              >
                日历
              </button>
              <button 
                className={`px-5 py-2 text-sm font-medium rounded-lg transition-all ${activeTab === 'meeting-room' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
                onClick={() => setActiveTab('meeting-room')}
              >
                会议室
              </button>
              <button 
                className={`px-5 py-2 text-sm font-medium rounded-lg transition-all ${activeTab === 'reservation' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
